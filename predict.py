...
def to_model_input(cv2_img, shape):
    image = Image.fromarray(cv2_img)
    width, height = image.size
    image = cv2.resize(cv2_img, shape)
    image = img_to_array(image)
    # scale pixel values to [0, 1]
    image = image.astype('float32')
    image /= 255.0
...
def cv2_draw_boxes(frame, v_boxes, v_labels, v_scores):
    for i in range(len(v_boxes)):
        box = v_boxes[i]
        # get coordinates
        y1, x1, y2, x2 = box.ymin, box.xmin, box.ymax, box.xmax
...
def process_frame(model, frame, image, image_w, image_h):
    ...
    end_time = time.time() - start_time
    print(f"decoding duration {end_time}")

    start_time = time.time()
    # correct the sizes of the bounding boxes for the shape of the image
    correct_yolo_boxes(boxes, image_h, image_w, input_h, input_w)
    end_time = time.time() - start_time
    print(f"correction duration {end_time}")

    start_time = time.time()
    # suppress non-maximal boxes
    do_nms(boxes, 0.5)
    end_time = time.time() - start_time
    print(f"nms duration {end_time}")
